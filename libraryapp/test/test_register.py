...
from libraryapp.routes.register import register_process
...
def test_existing_username(test_session):
    add_user(name='tester',email="test@gmail.com",phone="08484822222", username='test', password='aB@123456789')
...
def test_update_user(sample_user):
    ...
    updated = User.query.filter(User.id==1).first()
    assert updated.name == "newname"
    assert updated.phone == "0848482273"
...
def test_update_invalid_phone(sample_user):
    ...
    assert updated.name == sample_user.name
    assert updated.phone == sample_user.phone
...
def test_auth_user_success(test_session, sample_user):
    password_raw = "Abc123@"
    password_hashed = str(hashlib.md5(password_raw.encode('utf-8')).hexdigest())

    sample_user.password = password_hashed
    sample_user.active = True
    test_session.commit()

    result = auth_user(sample_user.username, password_raw)

    assert result is not None
    assert result.username == sample_user.username

def test_auth_user_wrong_pass(sample_user):
    result = auth_user("tester", "345")
    assert result is None

def test_auth_user_wrong_username(sample_user):
    result = auth_user("testuser1", "Abc123@")
    assert result is None

def test_change_password(mocker):
    user = mocker.MagicMock()

    mocker.patch("libraryapp.dao.users.hash_password", return_value="hashed123")

    mock_commit = mocker.patch("libraryapp.dao.users.db.session.commit")
...
def test_register_view(mocker, test_client):
    mocker.patch("libraryapp.routes.register.render_template", return_value="OK")
